...
{
    public partial class Extra_Curricular_Activities : System.Web.UI.Page
    {
        ...
        protected void Insert_Btn_Click(object sender, EventArgs e)
        {
            ...
            {
                string checkRecord = "select count(*) from Extra_Curricular_Activities_COLLEGEPROJECT_FILE_UPLOAD where Year = '" + Year_Text.Text + "' AND Activity_Name = '" + Activity_Name_Text.Text + "' AND Teacher_ID = '" + IDLabel.Text + "' AND Convener_Member = '" + Convener_Member_Text.Text + "' AND Contribution = '" + Contribution_Text.Text + "';";
                ...
                else
                {
                    using (SqlConnection con = new SqlConnection(strCon))
                    {
                        ...
                        {
                            cmd.CommandText = "insert into Extra_Curricular_Activities_COLLEGEPROJECT_FILE_UPLOAD(Teacher_ID, Year, Activity_Name, Convener_Member, Contribution, UploadedOn) values(@Teacher_ID, @Year, @Activity_Name, @Convener_Member, @Contribution, @UploadedOn)";
        ...
        private void BindGridviewData()
        {
            ...
            GridView1.DataSource = LecturerDataAccessLayerExtra_Curricular_Activities.GetAllDetailsOfTheLecturers(Teacher_ID);
            GridView1.DataBind();
        }
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                LecturerDataAccessLayerExtra_Curricular_Activities.DeleteRecord(Convert.ToInt32(e.CommandArgument));
                BindGridviewData();
            ...
            else if (e.CommandName == "UpdateRow")
            {
                ...
                LecturerDataAccessLayerExtra_Curricular_Activities.UpdateRecord(S_No, Year, Activity_Name, Convener_Member, Contribution);
        ...
        protected void SearchBtn_Click(object sender, EventArgs e)
        {
            using (SqlConnection con = new SqlConnection(strCon))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    {
                        sql = "SELECT * FROM Extra_Curricular_Activities_COLLEGEPROJECT_FILE_UPLOAD WHERE Teacher_ID = '" + IDLabel.Text + "' AND( Year LIKE @SearchParameter + '%' OR Activity_Name LIKE @SearchParameter + '%' OR Convener_Member LIKE @SearchParameter + '%' OR Contribution LIKE @SearchParameter + '%' );";
                        cmd.Parameters.AddWithValue("@SearchParameter", SearchTxt.Text.Trim());
                    }
                    else
                    {
                        sql = "SELECT * FROM Extra_Curricular_Activities_COLLEGEPROJECT_FILE_UPLOAD WHERE Teacher_ID = '" + IDLabel.Text + "';";
                    }
                    cmd.CommandText = sql;
                    cmd.Connection = con;
