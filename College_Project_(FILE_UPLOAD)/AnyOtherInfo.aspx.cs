...
{
    public partial class AnyOtherInfo : System.Web.UI.Page
    {
        ...
        protected void Insert_Btn_Click(object sender, EventArgs e)
        {
            ...
            {
                string checkRecord = "select count(*) from AnyOtherInfo_COLLEGEPROJECT_FILE_UPLOAD where Year = '" + Year_Text.Text + "' AND InfoName = '" + InfoName_Text.Text + "' AND Teacher_ID = '" + IDLabel.Text + "' AND Comment = '" + Comment_Text.Text + "';";
                    ...
                    if (FileUpload1.HasFile)
                    {
                        ...
                        if (filesize < 20728650)    //20MB
                        {
                            ...
                            using (SqlConnection con = new SqlConnection(strCon))
                            {
                                ...
                                {
                                    cmd.CommandText = "insert into AnyOtherInfo_COLLEGEPROJECT_FILE_UPLOAD(Teacher_ID, Year, InfoName, Comment, FileName, FileType, FileData, UploadedOn) values(@Teacher_ID, @Year, @InfoName, @Comment, @FileName, @FileType, @FileData, @UploadedOn)";

                                    cmd.Parameters.AddWithValue("@Teacher_ID", IDLabel.Text);
                                    cmd.Parameters.AddWithValue("@Year", Year_Text.Text);
                                    cmd.Parameters.AddWithValue("@InfoName", InfoName_Text.Text.ToUpper());
        ...
        private void BindGridviewData()
        {
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(strCon))
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        cmd.CommandText = "SELECT * FROM AnyOtherInfo_COLLEGEPROJECT_FILE_UPLOAD WHERE Teacher_ID =  @Teacher_ID";
                        cmd.Parameters.AddWithValue("@Teacher_ID", IDLabel.Text);
                        cmd.Connection = con;
                        con.Open();
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                LecturerDataAccessLayerAnyOtherInfo.DeleteRecord(Convert.ToInt32(e.CommandArgument));
                BindGridviewData();
            ...
            else if (e.CommandName == "UpdateRow")
            {
                ...
                string InfoName = ((TextBox)GridView1.Rows[rowIndex].FindControl("InfoNameEditTxt")).Text;
                ...
                LecturerDataAccessLayerAnyOtherInfo.UpdateRecord(S_No, Year, InfoName, Comment, FileName);
        ...
        protected void SearchBtn_Click(object sender, EventArgs e)
        {
            using (SqlConnection con = new SqlConnection(strCon))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    {
                        sql = "SELECT * FROM AnyOtherInfo_COLLEGEPROJECT_FILE_UPLOAD WHERE Teacher_ID = '" + IDLabel.Text + "' AND( Year LIKE @SearchParameter + '%' OR InfoName LIKE @SearchParameter + '%' OR Comment LIKE @SearchParameter + '%' OR FileName LIKE @SearchParameter + '%' );";
                        cmd.Parameters.AddWithValue("@SearchParameter", SearchTxt.Text.Trim());
                    }
                    else
                    {
                        sql = "SELECT * FROM AnyOtherInfo_COLLEGEPROJECT_FILE_UPLOAD WHERE Teacher_ID = '" + IDLabel.Text + "';";
                    }
                    cmd.CommandText = sql;
                    cmd.Connection = con;
