...
{
    public partial class Organized_Table_1_Table : System.Web.UI.Page
    {
        ...
        protected void Insert_Btn_Click(object sender, EventArgs e)
        {
            ...
            {
                string checkRecord = "select count(*) from Organized_Table_1_COLLEGEPROJECT_FILE_UPLOAD where Year = '" + YearTxt.Text + "' AND Theme = '" + ThemeTxt.Text + "' AND Teacher_ID = '" + IDLabel.Text + "' AND Date_Value = '" + CalendarTxt.Text + "' AND Level = '" + LevelTxt.Text + "' AND YourRole = '" + YourRoleTxt.Text + "';";
                    ...
                    if (FileUpload1.HasFile)
                    {
                        ...
                        if (filesize < 20728650)    //20MB
                        {
                            ...
                            using (SqlConnection con = new SqlConnection(strCon))
                            {
                                ...
                                {
                                    cmd.CommandText = "insert into Organized_Table_1_COLLEGEPROJECT_FILE_UPLOAD(Teacher_ID, Year, Theme, Date_Value, Level, YourRole, FileName, FileType, FileData, UploadedOn) values(@Teacher_ID, @Year, @Theme, @Date_Value, @Level, @YourRole, @FileName, @FileType, @FileData, @UploadedOn)";
                                    ...
                                    cmd.Parameters.AddWithValue("@YourRole", YourRoleTxt.Text.ToUpper());
        ...
        private void BindGridviewData()
        {
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(strCon))
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        cmd.CommandText = "SELECT * FROM Organized_Table_1_COLLEGEPROJECT_FILE_UPLOAD WHERE Teacher_ID =  @Teacher_ID";
                        cmd.Parameters.AddWithValue("@Teacher_ID", IDLabel.Text);
                        cmd.Connection = con;
                        con.Open();
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                LecturerDataAccessLayerOrganized_Table_1.DeleteRecord(Convert.ToInt32(e.CommandArgument));
                BindGridviewData();
            ...
            else if (e.CommandName == "UpdateRow")
            {
                ...
                string YourRole = ((TextBox)GridView1.Rows[rowIndex].FindControl("YourRoleEditTxt")).Text;

                string FileName = ((TextBox)GridView1.Rows[rowIndex].FindControl("EditFileName")).Text;

                LecturerDataAccessLayerOrganized_Table_1.UpdateRecord(S_No, Year, Theme, Date_Value, Level, YourRole, FileName);
        ...
        protected void SearchBtn_Click(object sender, EventArgs e)
        {
            using (SqlConnection con = new SqlConnection(strCon))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    {
                        sql = "SELECT * FROM Organized_Table_1_COLLEGEPROJECT_FILE_UPLOAD WHERE Teacher_ID = '" + IDLabel.Text + "' AND( Year LIKE @SearchParameter + '%' OR Theme LIKE @SearchParameter + '%' OR Date_Value LIKE @SearchParameter + '%' OR Level LIKE @SearchParameter + '%' OR YourRole LIKE @SearchParameter + '%' OR FileName LIKE @SearchParameter + '%' );";
                        cmd.Parameters.AddWithValue("@SearchParameter", SearchTxt.Text.Trim());
                    }
                    else
                    {
                        sql = "SELECT * FROM Organized_Table_1_COLLEGEPROJECT_FILE_UPLOAD WHERE Teacher_ID = '" + IDLabel.Text + "';";
                    }
                    cmd.CommandText = sql;
                    cmd.Connection = con;
