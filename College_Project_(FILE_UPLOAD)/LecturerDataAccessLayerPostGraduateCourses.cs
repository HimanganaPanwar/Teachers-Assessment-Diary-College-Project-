...
{
    public class Lecturer_PostGraduateCourses
    {
    ...
    public class LecturerDataAccessLayerPostGraduateCourses
    {
        static string cs = ConfigurationManager.ConnectionStrings["DBCS"].ConnectionString;

        //SELECT METHOD:(RETRIEVE ALL THE RECORDS OF A PARTICULAR LECTURER FROM THE TABLE)
        public static List<Lecturer_PostGraduateCourses> GetAllDetailsOfTheLecturers(int Teacher_ID)
        {
            List<Lecturer_PostGraduateCourses> listLecturers = new List<Lecturer_PostGraduateCourses>();

            using (SqlConnection con = new SqlConnection(cs))
            {
                SqlCommand cmd = new SqlCommand("SELECT * FROM PostGraduateCourses_COLLEGEPROJECT_FILE_UPLOAD WHERE Teacher_ID =  @Teacher_ID", con);
                ...
                while (rdr.Read())
                {
                    Lecturer_PostGraduateCourses lecturer = new Lecturer_PostGraduateCourses();
        ...
        public static void DeleteRecord(int S_No)
        {
            using (SqlConnection con = new SqlConnection(cs))
            {
                SqlCommand cmd = new SqlCommand("DELETE FROM PostGraduateCourses_COLLEGEPROJECT_FILE_UPLOAD WHERE S_No = @S_No", con);
        ...
        public static int UpdateRecord(int S_No, int Year, string Class, string Paper, string Nomenclature, int PeriodsPerWeek, string Lecture_Method, string Seminar_Presentation_Method)
        {
            using (SqlConnection con = new SqlConnection(cs))
            {
                string updateQuery = "UPDATE PostGraduateCourses_COLLEGEPROJECT_FILE_UPLOAD SET Year = @Year, " + "Class = @Class, Paper = @Paper, Nomenclature = @Nomenclature, PeriodsPerWeek = @PeriodsPerWeek, Lecture_Method = @Lecture_Method, Seminar_Presentation_Method = @Seminar_Presentation_Method WHERE S_No= @S_No";
