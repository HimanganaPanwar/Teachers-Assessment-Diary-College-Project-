...
{
    public class Lecturer_Co_Curricular_Activities
    {
    ...
    public class LecturerDataAccessLayerCo_Curricular_Activities
    {
        static string cs = ConfigurationManager.ConnectionStrings["DBCS"].ConnectionString;

        //SELECT METHOD:(RETRIEVE ALL THE RECORDS OF A PARTICULAR LECTURER FROM THE TABLE)
        public static List<Lecturer_Co_Curricular_Activities> GetAllDetailsOfTheLecturers(int Teacher_ID)
        {
            List<Lecturer_Co_Curricular_Activities> listLecturers = new List<Lecturer_Co_Curricular_Activities>();

            using (SqlConnection con = new SqlConnection(cs))
            {
                SqlCommand cmd = new SqlCommand("SELECT * FROM Co_Curricular_Activities_COLLEGEPROJECT_FILE_UPLOAD WHERE Teacher_ID =  @Teacher_ID", con);
                ...
                while (rdr.Read())
                {
                    Lecturer_Co_Curricular_Activities lecturer = new Lecturer_Co_Curricular_Activities();
                    ...
                    lecturer.Helped = rdr["Helped"].ToString();
        ...
        public static void DeleteRecord(int S_No)
        {
            using (SqlConnection con = new SqlConnection(cs))
            {
                SqlCommand cmd = new SqlCommand("DELETE FROM Co_Curricular_Activities_COLLEGEPROJECT_FILE_UPLOAD WHERE S_No = @S_No", con);
        ...
        //UPDATE METHOD: 
        public static int UpdateRecord(int S_No, int Year, string Activity_Name, string Convener_Member, string Contribution, string Helped)
        {
            using (SqlConnection con = new SqlConnection(cs))
            {
                string updateQuery = "UPDATE Co_Curricular_Activities_COLLEGEPROJECT_FILE_UPLOAD SET Year = @Year, " + "Activity_Name = @Activity_Name, Convener_Member = @Convener_Member, Contribution = @Contribution, Helped = @Helped WHERE S_No= @S_No";
                ...
                SqlParameter paramHelped = new SqlParameter("@Helped", Helped);
                cmd.Parameters.Add(paramHelped);
