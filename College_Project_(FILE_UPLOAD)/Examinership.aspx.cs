...
{
    public partial class Examinership : System.Web.UI.Page
    {
        ...
        protected void Insert_Btn_Click(object sender, EventArgs e)
        {
            ...
            {
                string checkRecord = "select count(*) from Examinership_COLLEGEPROJECT_FILE_UPLOAD where Class = '" + Class_Text.Text + "' AND Paper = '" + Paper_Text.Text + "' AND Institution_Name = '" + Institution_Name_Text.Text + "' AND Teacher_ID = '" + IDLabel.Text + "';";
                    ...
                    if (FileUpload1.HasFile)
                    {
                        ...
                        if (filesize < 20728650)    //20MB
                        {
                            ...
                            using (SqlConnection con = new SqlConnection(strCon))
                            {
                                ...
                                {
                                    cmd.CommandText = "insert into Examinership_COLLEGEPROJECT_FILE_UPLOAD(Teacher_ID, Year, Class, Paper, Institution_Name, FileName, FileType, FileData, UploadedOn) VALUES(@Teacher_ID, @Year, @Class, @Paper, @Institution_Name, @FileName, @FileType, @FileData, @UploadedOn)";

                                    cmd.Parameters.AddWithValue("@Teacher_ID", IDLabel.Text);
                                    cmd.Parameters.AddWithValue("@Year", Year_Text.Text);
                                    cmd.Parameters.AddWithValue("@Class", Class_Text.Text.ToUpper());
                                    cmd.Parameters.AddWithValue("@Paper", Paper_Text.Text.ToUpper());
                                    cmd.Parameters.AddWithValue("@Institution_Name", Institution_Name_Text.Text.ToUpper());
        ...
        private void BindGridviewData()
        {
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(strCon))
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        cmd.CommandText = "SELECT * FROM Examinership_COLLEGEPROJECT_FILE_UPLOAD WHERE Teacher_ID =  @Teacher_ID";
                        cmd.Parameters.AddWithValue("@Teacher_ID", IDLabel.Text);
                        cmd.Connection = con;
                        con.Open();
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                LecturerDataAccessLayerExaminership.DeleteRecord(Convert.ToInt32(e.CommandArgument));
                BindGridviewData();
            ...
            else if (e.CommandName == "UpdateRow")
            {
                ...
                string Institution_Name = ((TextBox)GridView1.Rows[rowIndex].FindControl("Institution_NameEditTxt")).Text;

                string FileName = ((TextBox)GridView1.Rows[rowIndex].FindControl("EditFileName")).Text;

                LecturerDataAccessLayerExaminership.UpdateRecord(S_No, Year, Class, Paper, Institution_Name, FileName);
        ...
        protected void SearchBtn_Click(object sender, EventArgs e)
        {
            using (SqlConnection con = new SqlConnection(strCon))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    {
                        sql = "SELECT * FROM Examinership_COLLEGEPROJECT_FILE_UPLOAD WHERE Teacher_ID = '" + IDLabel.Text + "' AND( Year LIKE @SearchParameter + '%' OR Class LIKE @SearchParameter + '%' OR Paper LIKE @SearchParameter + '%' OR Institution_Name LIKE @SearchParameter + '%' OR FileName LIKE @SearchParameter + '%' );";
                        cmd.Parameters.AddWithValue("@SearchParameter", SearchTxt.Text.Trim());
                    }
                    else
                    {
                        sql = "SELECT * FROM Examinership_COLLEGEPROJECT_FILE_UPLOAD WHERE Teacher_ID = '" + IDLabel.Text + "';";
                    }
                    cmd.CommandText = sql;
                    cmd.Connection = con;
