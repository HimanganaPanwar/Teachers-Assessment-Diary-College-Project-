...
{
    public partial class Publications_Projects_Table : System.Web.UI.Page
    {
        ...
        protected void Insert_Btn_Click(object sender, EventArgs e)
        {
            ...
            {
                string checkRecord = "select count(*) from Publications_Projects_COLLEGEPROJECT_FILE_UPLOAD where Title = '" + Title_Text.Text + "' AND Year = '" + Year_Text.Text + "' AND Teacher_ID = '" + IDLabel.Text + "' AND FundingAgency = '" + FundingAgency.Text + "' AND Complete = '" + Complete_Text.Text + "' AND Continuing = '" + Continuing_Text.Text + "' AND Major = '" + Major_Text.Text + "' AND Minor = '" + Minor_Text.Text + "' AND Other = '" + Other_Text.Text + "';";
                    ...
                    if (FileUpload1.HasFile)
                    {
                        ...
                        if (filesize < 20728650)    //20MB
                        {
                            ...
                            using (SqlConnection con = new SqlConnection(strCon))
                            {
                                ...
                                {
                                    cmd.CommandText = "insert into Publications_Projects_COLLEGEPROJECT_FILE_UPLOAD(Teacher_ID, Year, Title, FundingAgency, Complete, Continuing, Major, Minor, Other, FileName, FileType, FileData, UploadedOn) values(@Teacher_ID, @Year, @Title, @FundingAgency, @Complete, @Continuing, @Major, @Minor, @Other, @FileName, @FileType, @FileData, @UploadedOn)";
                                    ...
                                    cmd.Parameters.AddWithValue("@FundingAgency", FundingAgency.Text.ToUpper());
                                    ...
                                    cmd.Parameters.AddWithValue("@Major", Major_Text.Text.ToUpper());
                                    cmd.Parameters.AddWithValue("@Minor", Minor_Text.Text.ToUpper());
                                    cmd.Parameters.AddWithValue("@Other", Other_Text.Text.ToUpper());
        ...
        private void BindGridviewData()
        {
            ...
            using (SqlConnection con = new SqlConnection(strCon))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    cmd.CommandText = "SELECT * FROM Publications_Projects_COLLEGEPROJECT_FILE_UPLOAD WHERE Teacher_ID =  @Teacher_ID";
                    cmd.Parameters.AddWithValue("@Teacher_ID", IDLabel.Text);
                    cmd.Connection = con;
                    con.Open();
        ...
        protected void GridView1_RowCommand1(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                LecturerDataAccessLayerPublications_Projects.DeleteRecord(Convert.ToInt32(e.CommandArgument));
                BindGridviewData();
            ...
            else if (e.CommandName == "UpdateRow")
            {
                ...
                string FundingAgency = ((TextBox)GridView1.Rows[rowIndex].FindControl("FundingAgencyEditTxt")).Text;
                ...
                string Major = ((TextBox)GridView1.Rows[rowIndex].FindControl("MajorEditTxt")).Text;

                string Minor = ((TextBox)GridView1.Rows[rowIndex].FindControl("MinorEditTxt")).Text;

                string Other = ((TextBox)GridView1.Rows[rowIndex].FindControl("OtherEditTxt")).Text;

                string FileName = ((TextBox)GridView1.Rows[rowIndex].FindControl("EditFileName")).Text;

                LecturerDataAccessLayerPublications_Projects.UpdateRecord(S_No, Year, Title, FundingAgency, Complete, Continuing, Major, Minor, Other, FileName);
        ...
        protected void SearchBtn_Click(object sender, EventArgs e)
        {
            using (SqlConnection con = new SqlConnection(strCon))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    {
                        sql = "SELECT * FROM Publications_Projects_COLLEGEPROJECT_FILE_UPLOAD WHERE Teacher_ID = '" + IDLabel.Text + "' AND( Year LIKE @SearchParameter + '%' OR Title LIKE @SearchParameter + '%' OR FundingAgency LIKE @SearchParameter + '%' OR Complete LIKE @SearchParameter + '%' OR Continuing LIKE @SearchParameter + '%' OR Major LIKE @SearchParameter + '%' OR Minor LIKE @SearchParameter + '%' OR Other LIKE @SearchParameter + '%' );";
                        cmd.Parameters.AddWithValue("@SearchParameter", SearchTxt.Text.Trim());
                    }
                    else
                    {
                        sql = "SELECT * FROM Publications_Projects_COLLEGEPROJECT_FILE_UPLOAD WHERE Teacher_ID = '" + IDLabel.Text + "';";
                    }
                    cmd.CommandText = sql;
                    cmd.Connection = con;
